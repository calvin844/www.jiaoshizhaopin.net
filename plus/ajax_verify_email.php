...
if ($act == "send_code") {
    ...
    if ($userinfo && $userinfo['uid'] <> $_SESSION['uid']) {
        ...
        if (smtp_mail($email, "{$_CFG['site_name']}邮件认证", "{$QISHI['site_name']}提醒您：<br>您正在进行邮箱验证，验证码为:<strong>{$rand}</strong>")) {
            ...
            exit("此邮箱服务商未通过验证，请更换邮箱服务商，建议使用QQ邮箱");
        }
    }
            ...
            if ($_CFG['operation_mode'] == '1' && $_SESSION['utype'] == '1') {
                ...
                if ($rule['verifyemail']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], " 邮箱通过验证，{$_CFG['points_byname']}({$operator}{$rule['verifyemail']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})", 1, 1015, "邮箱认证通过", "{$operator}{$rule['verifyemail']['value']}", "{$user_points}");
                    }
                }
                ...
                if ($rule['train_verifyemail']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 4, 9101, $_SESSION['username'], " 邮箱通过验证，{$_CFG['train_points_byname']}({$operator}{$rule['train_verifyemail']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
                    }
                }
                ...
                if ($rule['hunter_verifyemail']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 3, 9201, $_SESSION['username'], " 邮箱通过验证，{$_CFG['hunter_points_byname']}({$operator}{$rule['hunter_verifyemail']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
                    }
                }
            }
