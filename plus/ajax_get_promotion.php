    ...
    if ($jobsid > 0 && $days > 0) {
        ...
        if (inserttable(table('promotion'), $setsqlarr)) {
            ...
            if ($_CFG['operation_mode'] == '1' && $pro_cat['cat_points'] > 0) {
                ...
                write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "{$pro_cat['cat_name']}：<strong>{$jobs['jobs_name']}</strong>，推广 {$days} 天，(-{$points})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
            } elseif ($_CFG['operation_mode'] == '2') {
