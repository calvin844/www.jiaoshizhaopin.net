            ...
            if ($_SESSION['utype'] == "1" && $_CFG['operation_mode'] == '1') {
                ...
                if ($rule['verifymobile']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], " 手机通过验证，{$_CFG['points_byname']}({$operator}{$rule['verifymobile']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})", 1, 1016, "手机认证通过", "{$operator}{$rule['verifymobile']['value']}", "{$user_points}");
                    }
                }
                ...
                if ($rule['train_verifymobile']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 4, 9101, $_SESSION['username'], " 手机通过验证，{$_CFG['train_points_byname']}({$operator}{$rule['train_verifymobile']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
                    }
                }
                ...
                if ($rule['hunter_verifymobile']['value'] > 0) {
                    ...
                    if (empty($info)) {
                        ...
                        write_memberslog($_SESSION['uid'], 3, 9201, $_SESSION['username'], " 手机通过验证，{$_CFG['hunter_points_byname']}({$operator}{$rule['hunter_verifymobile']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
                    }
                }
            }
