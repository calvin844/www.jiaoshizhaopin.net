function resumelist()
{
    ...
    if (key)
    {
        ...
        {
            if (key_arr[x])
                $('.striking').highlight(key_arr[x]);
        }
    }
    $(".list:odd").css("background-color", "#F4F5FB");
    $(".titsub").hover(function() {
        $(this).addClass("titsub_h");
    }, function() {
        $(this).removeClass("titsub_h");
    });
    function setlistbg()
    {
    ...
    //全选反选
    $("input[name='selectall']").unbind().click(function() {
        $("#formresumelist :checkbox").attr('checked', $(this).attr('checked'))
    });
    //点击选择后重新加样式
    $("#formresumelist input[type='checkbox']").click(function() {
        setlistbg();
    });
}
...
function favorites(ajaxurl)
{
    ...
    $(".allfavorites").click(function()
    {
        ...
        else
        {
            ...
            $("#formresumelist .list :checkbox[checked]").each(function(index) {
