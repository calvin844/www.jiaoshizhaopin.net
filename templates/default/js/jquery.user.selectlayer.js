function allaround(dir) {
    ...
    if ($("#divJobCate").length > 0) {
        ...
        var arr_tmp = intention_jobs.substr(-1);
        if (arr_tmp == ",") {
            intention_jobs = intention_jobs.substr(0, intention_jobs.length - 1);
        }
        var recoverJobArray = intention_jobs.split(",");
        var jobText = "";
        $.each(recoverJobArray, function(index, val) {
            if(val.indexOf("-")>0){
                var demojobArray = val.split("-");
                var demojobItem = demojobArray[1];
            }else{
                var demojobItem = val;
            }
            if (demojobItem != "") {
                jobText = jobText + demojobItem + ","
            }
        });
        var tmp = jobText.substr(-1);
        if (tmp == ",") {
            jobText = jobText.substr(0, jobText.length - 1);
        }
        if(jobText != "undefined"){
            $("#jobText").attr("title", jobText);
            $("#jobText").html(jobText);
        }
        // 恢复职位选中条件
        /*
        ...
        if (pval) {
            ...
            $.each(sjobsArray, function(sindex, sval) {
                if (sval) {
                    ...
                    jobstr += '<li>';
                    jobstr += '<p><font><a rcoid="' + sjobs[0] + '" pid="0.' + jobs[0] + '.' + sjobs[0] + '" title="' + sjobs[1] + '" href="javascript:;">' + sjobs[1] + '</a></font></p>';
                    if (QS_jobs[sjobs[0]]) {
