...
foreach ($rs as $rs) {
    ...
    if ($userinfo['utype'] != 2) {
        ...
        echo "ID为:" . $rs['NCID'] . "，用户名为：" . escape_str($rs['UserName']) . " 的用户用户类型为企业，不需转换简历。<br/>";
        continue;
    ...
    if ((!empty($has_id)) || (!empty($has_id_tmp))) {
        echo "ID为:" . $rs['NCID'] . "，用户名为：" . escape_str($rs['UserName']) . " 的用户已存在<br/>";
        del_resume($rs['NCID']);
    }
...
//$jobs = $rs['WorkWillClass1'] . ',' . $rs['WorkWillClass2'] . ',' . $rs['WorkWillClass3'] . ',' . $rs['WorkWill1'] . ',' . $rs['WorkWill2'] . ',' . $rs['WorkWill3'];
    ...
    echo "ID为:" . $rs['NCID'] . "，用户名为：" . escape_str($rs['UserName']) . " 的用户转换成功！<br/>";
    unset($setsqlarr);
    sleep(1);
...
$html.='<title>简历转换</title>';
$html.='</head>';
$html.='<body>';
$m = $m - 1;
$t = intval($_GET['t']) + $m;
$html.='成功转换' . $size . '份简历(总计转换了' . $t . '份)，请不要关闭或离开页面，3秒后进入下' . $size . '份简历的转换，id标记(' . $thisid . ')';
