...
$sql = "select  top " . $size . " *  from pH_Job_Base where JobId>" . $id . " and Cityid=1037 ORDER BY JobId asc ";
$rs = ms_getall($sql);
;   //执行搜索语句
...
foreach ($rs as $rs) {
    ...
    if ($rs['JobName']) {
        ...
        //$sql = "select id from ".table('jobs')." where uid='{$userinfo['uid']}' AND jobs_name='".escape_str($rs['JobName'])."' ";
        $sql = "select * from " . table('jobs') . " where id=" . intval($rs['JobId']);
        //echo $sql."<br/>";
        $jobinfo = $db->getone($sql);

        $sql = "select * from " . table('jobs_tmp') . " where id=" . intval($rs['JobId']);
        $jobtmpinfo = $db->getone($sql);

        //计算投递量_Calvin20140624
        $numinfo = ms_getone("SELECT COUNT(*) as sum FROM pH_Company_InBox WHERE JobId=" . $thisid);
        if ($numinfo['sum'] > 300) {
            //过大时分段独立处理
            $big = $big . $thisid . ",";
            continue;
        ...
        //当记录已存在时，判断投递记录是否插入完整，如不完整，重新导入_Calvin_20140624
        if (!empty($jobinfo) || !empty($jobtmpinfo)) {
            del_jobs($thisid);
            del_jobs_app($thisid);
            $jobinfo = "";
            $jobtmpinfo = "";
        ...
        if (empty($jobinfo) && empty($jobtmpinfo) && !empty($company_profile['companyname'])) {
            ...
            //注意，复查
            $category = get_jobs_category_arr($rs['JobClass']);
            ...
            $db->query("Delete from " . table('jobs_search_key') . " WHERE id='" . $pid . "'");
            ...
            $db->query("Delete from " . table('jobs_search_tag') . " WHERE id='" . $pid . "'");
            inserttable(table('jobs_search_tag'), $tagsql);
            echo intval($rs['JobId']) . " 完成! - " . $numinfo['sum'] . "<br/>";
        } else {
            if ($jobinfo['companyname'] == "" && empty($jobtmpinfo) && !empty($company_profile['companyname'])) {
                echo intval($rs['JobId']) . " 为空!";
                exit;
                $setsqlarr['companyname'] = escape_str($company_profile['companyname']);
                $wheresqlarr = " id = " . intval($rs['JobId']);
                updatetable(table('jobs'), $setsqlarr, $wheresqlarr);
                echo intval($rs['JobId']) . " 更新完成!";
            } elseif ($jobtmpinfo['companyname'] == "" && empty($jobinfo) && !empty($company_profile['companyname'])) {
                echo intval($rs['JobId']) . " 为空!";
                exit;
                $setsqlarr['companyname'] = escape_str($company_profile['companyname']);
                $wheresqlarr = " id = " . intval($rs['JobId']);
                updatetable(table('jobs_tmp'), $setsqlarr, $wheresqlarr);
                echo intval($rs['JobId']) . " 更新完成!";
            }
            echo intval($rs['JobId']) . " 已存在! - " . $numinfo['sum'] . "<br/>";
            //echo $big."<br/>";
        }
    }
    //exit;
...
if (empty($thisid)) {
    $url_in_arr = array("url" => "conversion_resume.php", "addtime" => strtotime("now"));
    $cons_id = inserttable(table('cons'), $url_in_arr, TRUE);
    if (!empty($cons_id)) {
        echo '所有职位已经全部转换完成！';
        echo '<script type="text/javascript" language="javascript">window.location.href="conversion_resume.php";</script> ';
    }
    exit("所有职位已经全部转换完成！");
}
...
function goon($m, $thisid, $big, $size) {
    ...
    $m = $m - 1;
    $t = intval($_GET['t']) + $m;
    $html.='成功转换' . $size . '个职位(总计转换了' . $t . '个)，请不要关闭或离开页面，1秒后进入下' . $size . '个职位的转换，id标记(' . $thisid . ')';
    $html.='<script type="text/javascript" language="javascript">';
    $html.='function reloadyemian()';
    $html.='{ ';
    $html.='window.location.href="conversion_jobs.php?t=' . $t . '&id=' . $thisid . '&big=' . trim($big, ",") . '&m=' . ($m + 1) . '"; ';
    $html.='} ';
    $html.='window.setTimeout("reloadyemian();",1000); ';
    $html.='</script> ';
    $html.='</body></html>';
    $url_in_arr = array("url" => "conversion_jobs.php?t=" . $t . "&id=" . $thisid . "&big=" . trim($big, ',') . "&m=" . ($m + 1), "addtime" => strtotime("now"));
    $cons_id = inserttable(table('cons'), $url_in_arr, TRUE);
    echo $html;
}
...
function forbig($big) {
    ...
    foreach ($big as $k => $v) {
        $thisid = $v;
        $sql = "select *  from pH_Job_Base where JobId={$v}";
        //echo $sql;exit;
        $big_rs = ms_getall($sql);
        require_once(QISHI_ROOT_PATH . 'include/splitword.class.php');
        $sp = new SPWord();
        if ($big_rs['JobName']) {
            $userinfo_rs = ms_getone("select TOP 1 UserName,NCID from pH_Company_Base WHERE Comid='" . $big_rs['Comid'] . "'");
            if (empty($userinfo_rs)) {
                echo $thisid . " 公司数据有误!";
                continue;
            } else {
            ...
            $sql = "select id from " . table('jobs') . " where id=" . intval($big_rs['JobId']);
            $jobinfo = $db->getone($sql);

            $sql = "select id from " . table('jobs_tmp') . " where id=" . intval($big_rs['JobId']);
            $jobtmpinfo = $db->getone($sql);
            if (!empty($jobinfo) || !empty($jobtmpinfo)) {
                del_jobs(intval($big_rs['JobId']));
                del_jobs_app(intval($big_rs['JobId']));
                $jobinfo = "";
                $jobtmpinfo = "";
            }
            if (empty($jobtmpinfo) && empty($jobinfo) && !empty($company_profile)) {
                $setsqlarr['id'] = intval($big_rs['JobId']);
                $setsqlarr['add_mode'] = 1;
                $setsqlarr['uid'] = $userinfo['uid'];
                $setsqlarr['jobs_name'] = escape_str($big_rs['JobName']);
                $setsqlarr['companyname'] = escape_str($company_profile['companyname']);
                $setsqlarr['company_id'] = $company_profile['id'];
                $setsqlarr['company_addtime'] = $company_profile['addtime'];
                $setsqlarr['company_audit'] = $company_profile['audit'];
                $setsqlarr['contents'] = escape_str($big_rs['Require']);
                $nature = get_jobs_nature($big_rs['JobType']);
                $setsqlarr['nature'] = $nature[0];
                $setsqlarr['nature_cn'] = $nature[1];
                $gender = get_sex($big_rs['Sex']);
                $setsqlarr['sex'] = $gender[0];
                $setsqlarr['sex_cn'] = $gender[1];
                $setsqlarr['amount'] = get_jobs_amount($big_rs['Number']);
                $setsqlarr['emergency'] = $rs['BestFlag'] ? 1 : 0;
                $category = get_jobs_category_arr($big_rs['JobClass']);
                $setsqlarr['category'] = $category['category'];
                $setsqlarr['subclass'] = $category['subclass'];
                $setsqlarr['category_cn'] = $category['category_cn'];
                $setsqlarr['trade'] = $company_profile['trade'];
                ...
                $setsqlarr['scale_cn'] = $company_profile['scale_cn'];
                //修改地区属性转换＿Calvin_20140618
                $cname = escape_str($big_rs['Work_City']);
                $aname = escape_str($big_rs['Work_Area']);
                if (empty($cname) || $cname == "不限" || $cname == "其他" || $cname == "其它") {
                    $cname = match_other_city($setsqlarr['id'], escape_str($rs['Address']), $aname);
                    if (!$cname) {
                        continue;
                ...
                $education = get_jobs_education($big_rs['Edus']);
                $setsqlarr['education'] = $education['id'];
                $setsqlarr['education_cn'] = $education['cn'];
                $experience = get_jobs_experience($big_rs['Works']);
                $setsqlarr['experience'] = $experience['id'];
                $setsqlarr['experience_cn'] = $experience['cn'];
                $wage = $big_rs['Deal'];
                $wage = explode('.', $wage);
                ...
                $setsqlarr['wage'] = $wage['id'];
                $setsqlarr['wage_cn'] = $wage['name'];
                $setsqlarr['graduate'] = $big_rs['JobType'] == 3 ? 1 : 0;
                //修改添加时间为最后刷新时间_Calvin_20140618
                $end_date = $big_rs['End_Date'];
                $end_date = explode(' ', $end_date);
                $add_date = $big_rs['LastUpdate_Time'];
                $add_date = explode(' ', $add_date);
                $setsqlarr['deadline'] = strtotime($rs['End_Date']);
                $setsqlarr['addtime'] = strtotime($rs['LastUpdate_Time']);
                $setsqlarr['refreshtime'] = strtotime($big_rs['LastUpdate_Time']);
                $setsqlarr['key'] = $setsqlarr['jobs_name'] . $setsqlarr['companyname'] . $setsqlarr['category_cn'] . $setsqlarr['district_cn'] . $setsqlarr['contents'];
                $setsqlarr['key'] = "{$setsqlarr['jobs_name']} {$setsqlarr['companyname']} " . $sp->extracttag($setsqlarr['key']);
                $setsqlarr['key'] = $sp->pad($setsqlarr['key']);
                $setsqlarr['subsite_id'] = 0;
                $setsqlarr['tpl'] = $company_profile['tpl'];
                $setsqlarr['map_x'] = empty($company_profile['map_x']) ? 0 : $company_profile['map_x'];
                $setsqlarr['map_y'] == empty($company_profile['map_y']) ? 0 : $company_profile['map_y'];
                $setsqlarr['click'] = $big_rs['ViewClicks'];
                $setsqlarr['audit'] = ($big_rs['JobFlag'] == 1) ? 1 : 2;
                $setsqlarr_contact['contact'] = escape_str($big_rs['ContactPerson']);
                $setsqlarr_contact['qq'] = "";
                $setsqlarr_contact['telephone'] = $big_rs['Phone'];
                $setsqlarr_contact['address'] = escape_str($big_rs['Address']);
                $setsqlarr_contact['email'] = escape_str($big_rs['Email']);
                $setsqlarr_contact['notify'] = 0;
                if ($setsqlarr['deadline'] < time() || $setsqlarr['audit'] != 1) {
                    $pid = inserttable(table('jobs_tmp'), $setsqlarr, true);
                } else {
                ...
                empty($pid) ? showmsg("添加失败！", 0) : '';
                //补充转换简历投递量_Calvin_20140619(需写在此)---begin
                $sql = "select  *  from pH_Company_InBox where JobId=" . $big_rs['JobId'];
                $sc_rs = ms_getall($sql);
                if (!empty($sc_rs)) {
                    foreach ($sc_rs as $sc_rs) {
                        //查询简历信息
                ...
                $db->query("Delete from " . table('jobs_search_tag') . " WHERE id='" . $pid . "'");
                inserttable(table('jobs_search_tag'), $tagsql);
                echo intval($big_rs['JobId']) . "完成！<br/>";
            } else {
                if ($jobinfo['companyname'] == "") {
                    $setsqlarr['companyname'] = escape_str($company_profile['companyname']);
                    $wheresqlarr = " id = " . intval($big_rs['JobId']);
                    updatetable(table('jobs'), $setsqlarr, $wheresqlarr);
                    echo intval($big_rs['JobId']) . " 更新完成!";
                } elseif ($jobtmpinfo['companyname'] == "") {
                    $setsqlarr['companyname'] = escape_str($company_profile['companyname']);
                    $wheresqlarr = " id = " . intval($big_rs['JobId']);
                    updatetable(table('jobs_tmp'), $setsqlarr, $wheresqlarr);
                    echo intval($big_rs['JobId']) . " 更新完成!";
                }
                echo intval($big_rs['JobId']) . "已存在！<br/>";
            }
        }
    }
}
