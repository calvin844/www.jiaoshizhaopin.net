...
$sql = "select top " . $size . " * from pH_Company_Base  where NCID > " . $id . " and Cityid=1037 order by NCID asc";
$rs = ms_getall($sql);
;   //执行搜索语句
...
if (empty($thisid)) {
//echo "全部完成";exit;
    $url_in_arr = array("url" => "conversion_user_personal.php", "addtime" => strtotime("now"));
    $cons_id = inserttable(table('cons'), $url_in_arr, TRUE);
    if (!empty($cons_id)) {
        echo '所有企业会员已经全部转换完成！';
        echo '<script type="text/javascript" language="javascript">window.location.href="conversion_user_personal.php";</script> ';
    }
    exit("所有企业会员已经全部转换完成！");
}
...
$html.='<title>企业会员</title>';
$html.='</head>';
$html.='<body>';
$m = $m - 1;
$t = intval($_GET['t']) + $m;
$html.='成功转换' . $size . '个企业会员(总计转换了' . $t . '个)，请不要关闭或离开页面，3秒后进入下' . $size . '个企业会员的转换，id标记(' . $thisid . ')';
$html.='<script type="text/javascript" language="javascript">';
$html.='function reloadyemian()';
$html.='{ ';
$html.='window.location.href="conversion_user_company.php?t=' . $t . '&id=' . $thisid . '"; ';
$html.='} ';
$html.='window.setTimeout("reloadyemian();",3000); ';
$html.='</script> ';
$html.='</body></html>';
$url_in_arr = array("url" => "conversion_user_company.php?t=" . $t . "&id=" . $thisid, "addtime" => strtotime("now"));
$cons_id = inserttable(table('cons'), $url_in_arr, TRUE);
echo $html;
