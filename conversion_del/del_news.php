...
if(!empty($city_rs['id'])){
    $sql = "select id from ".table("article")." WHERE `seo_description` LIKE '%".$city_rs['categoryname']."%'";
    ...
    //$db->query("Delete from ".table('article_category')." WHERE id='{$city_rs['id']}' LIMIT 1");
}else{
    exit("参数错误！");
}
exit("所有新闻已经全部处理完成！");
