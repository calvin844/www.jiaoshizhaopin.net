...
$sql = "select * from ".table("WEBSITE_AREA")." where area_level = 1";
$area_res = $db->getall($sql);
foreach ($area_res as $ar) {
    $sql = "select * from ".table("category_district")." where categoryname = '".substr($ar['area_name'],0,strlen($ar['area_name'])-2)."' limit 1";
    $qs_area_res = $db->getone($sql);
    $sql = "select * from ".table("WEBSITE_AREA")." where area_level = ".($ar['area_level']+1)." and parent_id = ".$ar['area_id']." and area_id < ".($ar['area_id']+999);
    $city_res = $db->getall($sql);
    if(count($city_res)<3){
        foreach ($city_res as $c) {
            $sql = "select * from ".table("WEBSITE_AREA")." where area_level = ".($ar['area_level']+2)." and parent_id = ".$c['area_id']." and area_id < ".($c['area_id']+99)." and area_id > ".$c['area_id'];
            $citys_res = $db->getall($sql);
            foreach ($citys_res as $csr) {
                $citys_tmp_arr[] = $csr;
            }
        }
        $city_res = $citys_tmp_arr;
    }
    foreach ($city_res as $cr) {
        $last_str = substr($cr['area_name'],-2);
        if($last_str == "市" || $last_str == "区" || $last_str == "县"){
            if(!strstr($cr['area_name'], "自治".$last_str)){
                $cr['area_name'] = substr($cr['area_name'],0,strlen($cr['area_name'])-2);
            }
        }
        $sql = "select * from ".table("category_district")." where categoryname = '".$cr['area_name']."' limit 1";
        $qs_city_res = $db->getone($sql);
        if(empty($qs_city_res['id'])){
            $in_arr['parentid'] = $qs_area_res['id'];
            $in_arr['categoryname'] = $cr['area_name'];
            $c_id = inserttable(table('category_district'),$in_arr,1);
