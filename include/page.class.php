...
class page {
    ...
    function page($array) {
        if (is_array($array)) {
            ...
            $url = (array_key_exists('url', $array)) ? $array['url'] : $_SERVER['REQUEST_URI'];
            if (array_key_exists('url', $array)) {
                $url = $array['url'];
            } else {
                $url_arr = explode($this->page_name, $_SERVER['REQUEST_URI']);
                $url = trim($url_arr[0], "&");
                $url = trim($url, "?");
            }
    ...
    function show($mode = 1) {
        ...
        switch ($mode) {
            ...
            case '4':
                ...
                return $this->pre_page() . "<span>" . $this->nowindex . "/" . $this->totalpage . "页</span>" . $this->next_page() . "<div class=\"clear\"></div>";
