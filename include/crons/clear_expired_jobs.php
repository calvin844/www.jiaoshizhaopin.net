...
global $_CFG;
$time = time();
//var_dump($_CFG['outdated_jobs']);exit;
if ($_CFG['outdated_jobs'] == "1") {
        ...
        if ((time() - $time) > 3) {
            if (!empty($did) && is_array($did)) {
                foreach ($did as $id) {
                    ...
                    $db->query("Delete from  " . table('jiaoshi_article_jobs_index') . " where p_id='" . intval($id) . "' and type = 'jobs' LIMIT 1");
    ...
    $db->query("Delete from  " . table('jiaoshi_article_jobs_index') . " WHERE parent_id NOT IN(select id from " . table("article") . ") and `type` = 'article'");
}
//更新任务时间表
if ($crons['weekday'] >= 0) {
