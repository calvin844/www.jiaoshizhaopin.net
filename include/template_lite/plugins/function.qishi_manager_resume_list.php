<?php
function tpl_function_qishi_manager_resume_list($params, &$smarty)
{
global $db,$_CFG;
...
$aset['showname']=isset($aset['showname'])?$aset['showname']:'QS_manager_resumeshow';
$aset['listpage']=isset($aset['listpage'])?$aset['listpage']:'QS_manager_resumelist';
...
if (!empty($aset['category']) || !empty($aset['subclass']) || !empty($aset['jobcategory']))
{
	...
	$joinsql="  INNER  JOIN  ( SELECT DISTINCT pid FROM ".table('manager_resume_jobs')." {$joinwheresql} )AS j ON  r.id=j.pid ";
}
...
{
	if ($_CFG['managerresumesearch_purview']=='2')
	{
	...
	$key=trim($aset['key']);
	if ($_CFG['managerresumesearch_type']=='1')
	{
		$akey=explode(' ',$key);
		if (count($akey)>1)
		{
	...
	else
	{
		...
		if ($_CFG['managerresumesearch_sort']=='1')
		{
		$orderbysql="";
		}
		else
...
$wheresql.=" AND r.display='1' AND r.audit=1 AND r.complete=1 ";
if (!empty($wheresql))
{
$wheresql=" WHERE ".ltrim(ltrim($wheresql),'AND');
		...
		if (isset($aset['paged']))
		{
			...
			$total_sql="SELECT  COUNT(*) AS num  FROM  ".table('manager_resume')." AS r ".$joinsql.$wheresql;
	...
	$list = $id = array();
	$result = $db->query("SELECT * FROM ".table('manager_resume')." AS r ".$joinsql.$wheresql.$orderbysql.$limit);
	//echo "SELECT * FROM ".table('manager_resume')." AS r ".$joinsql.$wheresql.$orderbysql.$limit;
