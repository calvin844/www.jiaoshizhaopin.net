...
function tpl_function_qishi_ad($params, &$smarty) {
    ...
    foreach ($arrset as $str) {
        ...
        switch ($a[0]) {
            ...
            case "地区中文":
                ...
                break;
            case "简章地区":
                $aset['news_district'] = $a[1];
                break;
            case "简章城市":
                $aset['news_sdistrict'] = $a[1];
                break;
        }
    }
    if (is_array($aset)) {
    ...
    $aset['row'] = isset($aset['row']) ? intval($aset['row']) : 100;
    ...
    $aset['news_district'] = isset($aset['news_district']) ? intval($aset['news_district']) : "";
    $aset['news_sdistrict'] = isset($aset['news_sdistrict']) ? intval($aset['news_sdistrict']) : "";
    ...
    if (empty($aset['district']) || $aset['district'] == "morejobs"|| $aset['district'] == "news_b_list") {
        ...
        $district_data = get_mem_cache($sql, "getone");
        $district = $district_data['parentid'] == 0 ? $district_data['id'] : $district_data['parentid'];
        $sdistrict = $district_data['parentid'] == 0 ? 0 : $district_data['id'];
    }
    if (!empty($aset['news_district'])) {
        $district = $aset['news_district'];
        $sdistrict = $aset['news_sdistrict'];
    }

    if ($district > 0) {
        $d_wheresql = " AND (district = 0 OR district=" . intval($district) . ") AND sdistrict = 0";
    } else {
        $d_wheresql = " AND district = 0 AND sdistrict = 0";
    }
    ...
    if ($aset['displayorder']) {
        ...
        $orderbysql = " ORDER BY show_order DESC,id DESC ";
    }
    $sql = "SELECT * FROM " . table('ad') . " " . $wheresql . $d_wheresql . $orderbysql . $limit;
    $result = $db->getall($sql);
    if (count($result) < $aset['row']) {
        $no_id = "";
        $tmp_id = array();
        $tmp_row = $aset['row'] - count($result);
        foreach ($result as $r) {
            $tmp_id[] = $r['id'];
        }
        if (!empty($tmp_id)) {
            $tmp_id_str = implode(",", $tmp_id);
            $no_id = " id NOT IN(" . $tmp_id_str . ") AND ";
        }
        $limit = " LIMIT " . $aset['start'] . ',' . $tmp_row;
        $d_wheresql = " AND" . $no_id . " district = " . $district . " AND sdistrict = 0";
        $sql = "SELECT * FROM " . table('ad') . " " . $wheresql . $d_wheresql . $orderbysql . $limit;
        ...
        if (count($result) < $aset['row']) {
            ...
            $limit = " LIMIT " . $aset['start'] . ',' . $tmp_row;
            $d_wheresql = " AND" . $no_id . " sdistrict = " . $sdistrict;
            $sql = "SELECT * FROM " . table('ad') . " " . $wheresql . $d_wheresql . $orderbysql . $limit;
            $result3 = $db->getall($sql);
    ...
    foreach ($result as $row) {
        ...
        if ($row['type_id'] == "1") {//文字
            ...
            $list['text_color'] = $row['text_color'];
            $list['note'] = $row['note'];
            ...
            $list['note'] = $row['note'];
            $list['index'] = $i++;
            /* if ($aset['alias']  == 'index_top_small_ad') {
              $arr = explode(',', $row['img_explain']);
              if (!empty($arr)) {
              $list['title'] = $arr[0];
              $list['content'] = $arr[1];
              }
