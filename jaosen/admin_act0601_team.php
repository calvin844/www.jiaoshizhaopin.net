...
if ($act == 'index') {
    ...
    if ($key > 0 && !empty($value)) {
        if ($key == 1) {
            ...
            $where .= " AND uid IN(SELECT id FROM " . table('resume') . " WHERE fullname LIKE '%" . $value . "%') ";
        } else if ($key == 3) {
            $where .= " AND leader IN(SELECT id FROM " . table('resume') . " WHERE fullname LIKE '%" . $value . "%') ";
        } else if ($key == 4) {
            $where .= " AND audit LIKE '%" . $value . "%'";
        }
    }
    $sql = "select count(*) as total from " . table('act_20170601_team') . $where;
    $total = $db->getone($sql);
    ...
    $sql = "select * from " . table('act_20170601_team') . $where . " LIMIT " . $offset . "," . $limit;
    $ilist = $db->getall($sql);
    foreach ($ilist as $ilist) {
        $sql = "select * from " . table('act_20170601') . " where uid = " . $ilist['leader'];
        $item = $db->getone($sql);
        $sql = "select * from " . table('resume') . " where uid = " . $ilist['uid'];
        $resume = $db->getone($sql);
        $sql = "select * from " . table('resume') . " where uid = " . $ilist['leader'];
        $leader_resume = $db->getone($sql);
        $ilist['item'] = $item;
        $ilist['leader_resume'] = $leader_resume;
        $ilist['resume'] = $resume;
        $data_list[] = $ilist;
    }
    ...
    $smarty->display('act0601/team_list.htm');
    ...
    $db->query("UPDATE " . table("act_20170601_team") . " SET audit = " . $audit . " WHERE id IN(" . $id_str . ")");
    echo '<script language="javascript" type="text/javascript">alert("更新成功！")</script>';
} elseif ($act == 'del_team') {
    $id_str = intval($_GET['id']);
    if ($id_str > 0) {
        $db->query("DELETE FROM " . table("act_20170601_team") . " WHERE id IN(" . $id_str . ")");
