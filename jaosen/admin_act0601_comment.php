...
if ($act == 'index') {
    ...
    $sql = "select count(*) as total from " . table('act_20170601_comment') . $where;
    $total = $db->getone($sql);
    ...
    $sql = "select * from " . table('act_20170601_comment') . $where . " LIMIT " . $offset . "," . $limit;
    $ilist = $db->getall($sql);
    foreach ($ilist as $ilist) {
        $sql = "select * from " . table('act_20170601') . " where id = " . $ilist['pid'];
        $item = $db->getone($sql);
        $sql = "select * from " . table('resume') . " where uid = " . $ilist['uid'];
        $resume = $db->getone($sql);
        $sql = "select * from " . table('resume') . " where uid = " . $item['uid'];
    ...
    $smarty->display('act0601/comment_list.htm');
    ...
    $db->query("UPDATE " . table("act_20170601_comment") . " SET audit = " . $audit . " WHERE id IN(" . $id_str . ")");
    echo '<script language="javascript" type="text/javascript">alert("更新成功！")</script>';
} elseif ($act == 'del_comment') {
    $id_str = intval($_GET['id']);
    if ($id_str > 0) {
        $db->query("DELETE FROM " . table("act_20170601_comment") . " WHERE id IN(" . $id_str . ")");
