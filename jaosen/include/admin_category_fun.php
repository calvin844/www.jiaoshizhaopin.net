...
function del_group($alias) {
    ...
    $return = 0;
    foreach ($alias as $a) {
        if (!$db->query("Delete from " . table('category_group') . " WHERE g_alias ='" . trim($a) . "' AND g_sys<>1"))
            return false;
        $return = $return + $db->affected_rows();
        if (!$db->query("Delete from " . table('category') . " WHERE c_alias ='" . trim($a) . "' "))
            return false;
        $return = $return + $db->affected_rows();
    }
    return $return;
...
function del_color($id) {
    global $db;
    $return = 0;
    if (!$db->query("Delete from " . table('color') . " WHERE id =" . intval($id)))
        return false;
    $return = $return + $db->affected_rows();
    return $return;
}
...
function del_category($id) {
    ...
    if (preg_match("/^(\d{1,10},)*(\d{1,10})$/", $sqlin)) {
        if (!$db->query("Delete from " . table('category') . " WHERE c_id IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
    }
    return $return;
...
function del_district($id) {
    ...
    $sqlin = implode(",", $id);
    if (preg_match("/^(\d{1,10},)*(\d{1,10})$/", $sqlin)) {
        if (!$db->query("Delete from " . table('category_district') . " WHERE id IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
        if (!$db->query("Delete from " . table('category_district') . " WHERE parentid IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
    }
    return $return;
...
function del_jobs_category($id) {
    ...
    $sqlin = implode(",", $id);
    if (preg_match("/^(\d{1,10},)*(\d{1,10})$/", $sqlin)) {
        if (!$db->query("Delete from " . table('category_jobs') . " WHERE id IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
        if (!$db->query("Delete from " . table('category_jobs') . " WHERE parentid IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
    }
    return $return;
...
function del_hunterjobs_category($id) {
    ...
    $sqlin = implode(",", $id);
    if (preg_match("/^(\d{1,10},)*(\d{1,10})$/", $sqlin)) {
        if (!$db->query("Delete from " . table('category_hunterjobs') . " WHERE id IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
        if (!$db->query("Delete from " . table('category_hunterjobs') . " WHERE parentid IN (" . $sqlin . ") "))
            return false;
        $return = $return + $db->affected_rows();
    }
    return $return;
