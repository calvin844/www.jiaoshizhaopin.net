...
if ($act == 'set') {
    ...
    foreach ($_POST as $k => $v) {
        $sql = "select id from " . table('config') . " where name='" . $k . "'";
        $r = $db->getone($sql);
        if ($r < 1) {
            $sql = "INSERT INTO " . table('config') . " (name) VALUES ( '$k')";
            $db->query($sql);
        }
        !$db->query("UPDATE " . table('config') . " SET value='$v' WHERE name='$k'") ? adminmsg('更新设置失败', 1) : "";
    }
    foreach ($_POST as $k => $v) {
        $sql = "select id from " . table('text') . " where name='" . $k . "'";
        $r = $db->getone($sql);
        if ($r < 1) {
            $sql = "INSERT INTO " . table('text') . " (name) VALUES ( '$k')";
            $db->query($sql);
        }
