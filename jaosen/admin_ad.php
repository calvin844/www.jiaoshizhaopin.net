...
elseif ($act == 'ad_add') {
    ...
    $smarty->assign('ad_district', $district);
...
elseif ($act == 'ad_add_save') {
    ...
    $setsqlarr['sdistrict'] = isset($_POST['sdistrict']) ? intval($_POST['sdistrict']) : 0;
    if ($setsqlarr['district'] == 0) {
        $setsqlarr['district'] = $setsqlarr['sdistrict'] = 0;
        $setsqlarr['district_cn'] = "全国";
    } else {
        ...
        if ($setsqlarr['sdistrict'] != 0) {
            ...
            $setsqlarr['district_cn'] = $district_cn . $sdistrict_cn;
        }
    }
    ...
    elseif ($setsqlarr['type_id'] == "5") {
        ...
        if ($setsqlarr['floating_left'] === "" && $setsqlarr['floating_right'] === "")
            adminmsg('左边距和右边距至少填写一项！', 1);
...
elseif ($act == 'edit_ad') {
    ...
    $smarty->assign('ad_district', $district);
    if ($ad['district'] > 0) {
        $sql = "select * from " . table('category_district') . " where parentid = " . $ad['district'];
        $sdistrict = $db->getall($sql);
    }
    $smarty->assign('ad_sdistrict', $sdistrict);
    $smarty->assign('ad', $ad);
...
elseif ($act == 'ad_category_edit_save') {
    ...
    $setsqlarr['expense'] = intval($_POST['expense']);
    $setsqlarr['expense_month'] = intval($_POST['expense_month']);
    $setsqlarr['expense_year'] = intval($_POST['expense_year']);
    $setsqlarr['expense_piece'] = intval($_POST['expense_piece']);
    $wheresql = " id='" . intval($_POST['id']) . "'";
...
} elseif ($act == 'ad_count_list') {
    $start_time = !empty($_GET['start_time']) ? " and click_time > " . strtotime($_GET['start_time']) : "";
    $end_time = !empty($_GET['end_time']) ? " and click_time < " . strtotime($_GET['end_time']) : "";
    $wheresql = " where 1 " . $start_time . $end_time;
    $count_list = get_adv_count_list($wheresql);
    $smarty->assign('pageheader', "广告位统计列表");
    $smarty->assign('start_time', $_GET['start_time']);
    $smarty->assign('end_time', $_GET['end_time']);
    $smarty->assign('count_list', $count_list);
    $smarty->display('ads/admin_adv_count_list.htm');
} elseif ($act == 'ad_count_info') {
    require_once(QISHI_ROOT_PATH . 'include/page.class.php');
    $ad_name = !empty($_GET['ad_name']) ? " and ad_name = '" . trim($_GET['ad_name']) . "'" : "";
    //$perpage = 10;
    $start_time = !empty($_GET['start_time']) ? " and click_time > " . strtotime($_GET['start_time']) : "";
    $end_time = !empty($_GET['end_time']) ? " and click_time < " . strtotime($_GET['end_time']) : "";
    $wheresql = " where 1 " . $ad_name . $start_time . $end_time;
    $total_sql = "SELECT COUNT(*) AS num FROM " . table('ad_count') . $wheresql;
    $total_val = $db->get_total($total_sql);
    $page = new page(array('total' => $total_val, 'perpage' => $perpage));
    $currenpage = $page->nowindex;
    $offset = ($currenpage - 1) * $perpage;
    $count_info = get_adv_count_one($offset, $perpage, $wheresql);
    $smarty->assign('pageheader', "广告位统计");
    $smarty->assign('start_time', $_GET['start_time']);
    $smarty->assign('end_time', $_GET['end_time']);
    $smarty->assign('count_info', $count_info);
    $smarty->assign('page', $page->show(3));
    $smarty->display('ads/admin_adv_count_info.htm');
}
//设置充值记录（收款开通）
