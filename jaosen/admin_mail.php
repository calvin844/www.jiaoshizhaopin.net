...
if ($act == 'email_set') {
    ...
    for ($i = 0; $i < count($mailconfig['smtpservers']); $i++) {
        if($mailconfig['smtpupdatetime'][$i] == 0 ){
            $mailconfig['smtpupdatetime'][$i] = date("Y-m-d", time());
        }else{
            $mailconfig['smtpupdatetime'][$i] = date("Y-m-d", $mailconfig['smtpupdatetime'][$i]);
        }
        $mailconfigli[] = array('smtpservers' => $mailconfig['smtpservers'][$i], 'smtpusername' => $mailconfig['smtpusername'][$i], 'smtppassword' => $mailconfig['smtppassword'][$i], 'smtpfrom' => $mailconfig['smtpfrom'][$i], 'smtpport' => $mailconfig['smtpport'][$i], 'smtpnum' => $mailconfig['smtpnum'][$i], 'smtpupdatetime' => $mailconfig['smtpupdatetime'][$i]);
    }
    ...
    if (intval($_POST['method']) == "1") {
        for ($i = 0; $i < count($_POST['smtpservers']); $i++) {
            if (empty($_POST['smtpservers'][$i]) || empty($_POST['smtpusername'][$i]) || empty($_POST['smtppassword'][$i]) || empty($_POST['smtpfrom'][$i]) || empty($_POST['smtpport'][$i]) || empty($_POST['smtpnum'][$i]) || empty($_POST['smtpupdatetime'][$i])) {
        ...
        $_POST['smtpnum'] = implode('|-_-|', $_POST['smtpnum']);
        $tmp = "";
        foreach ($_POST['smtpupdatetime'] as $s) {
            $tmp .= strtotime($s)."|-_-|";
        }
        $tmp = trim($tmp, "|-_-|");
        $_POST['smtpupdatetime'] = $tmp;
    }
...
if ($act == 'testing') {
    ...
    if (!preg_match("/^[\w\-\.]+@[\w\-\.]+(\.\w+)+$/", $check_smtp))
        adminmsg('email格式错误！', 1);
    if (smtp_mail($check_smtp, "教师招聘网测试邮件", $txt)) {
        adminmsg('测试邮件发送成功！', 2);
    } else {
        adminmsg('测试邮件发送失败！', 1);
    }
