...
elseif ($act == 'jobs') {
    ...
    $setsqlarr['categoryname'] = !empty($_POST['categoryname']) ? trim($_POST['categoryname']) : adminmsg("请填写名称", 1);
    $setsqlarr['category_order'] = intval($_POST['category_order']);
    $setsqlarr['pinyin'] = trim($_POST['pinyin']);
...
} elseif ($act == 'subscribelist') {
    get_token();
    $smarty->assign('navlabel', "subscribe_tag");
    $smarty->assign('subscribe_tags', get_subscribe_tags());
    $smarty->display('category/admin_subscribe_tag.htm');
}
?>