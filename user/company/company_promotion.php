...
if ($act == 'tpl') {
    ...
    if ($_CFG['operation_mode'] == '1') {
        if ($comtpl['tpl_val'] > 0) {
            ...
            $user_limit_points = get_user_limit_points($_SESSION['uid']);
            $user_limit_points = empty($user_limit_points['points']) ? 0 : $user_limit_points['points'];
            write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "设置企业模版：{$comtpl['tpl_name']}，(-{$comtpl['tpl_val']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})", 1, 1022, "选择模板", "-{$comtpl['tpl_val']}", "{$user_points}");
        }
    } elseif ($_CFG['operation_mode'] == '2' || $_CFG['operation_mode'] == '3') {
    ...
    if (!empty($setsqlarr)) {
        if (!updatetable(table('promotion'), $setsqlarr, " cp_id='{$id}' AND cp_uid='{$_SESSION['uid']}' "))
            showmsg("保存失败！", 0);
        if ($days > 0) {
            if ($_CFG['operation_mode'] == '1' && $catinfo['cat_points'] > 0) {
                ...
                write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "延长推广有效期{$days}天，(推广类型：{$catinfo['cat_name']}，推广职位：{$jobs['jobs_name']})，(-{$points})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})", 1, 1018, "延长推广", "-{$points}", "{$user_points}");
