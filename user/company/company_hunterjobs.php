    ...
    if ($_SESSION['addrand'] == $addrand) {
        ...
        if ($_CFG['operation_mode'] == '1') {
            if ($points_rule['hunterjobs_add']['value'] > 0) {
                ...
                write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "发布了高级职位：<strong>{$setsqlarr['jobs_name']}</strong>，({$operator}{$points_rule['hunterjobs_add']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
            }
            if (intval($_POST['days']) > 0 && $points_rule['hunterjobs_daily']['value'] > 0) {
                ...
                write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "发布高级职位:<strong>{$_POST['jobs_name']}</strong>，有效期为{$_POST['days']}天，({$operator}{$points_day})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
            }
        } elseif ($_CFG['operation_mode'] == '2') {
    ...
    if ($refresh) {
        if ($_CFG['operation_mode'] == '1') {
            ...
            if ($points_rule['hunterjobs_refresh']['value'] > 0) {
                ...
                write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "刷新了{$jobs_num}条高级职位，({$operator}{$total_point})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
            }
        }
...
elseif ($act == 'edit_hunterjobs_save') {
    ...
    if ($_CFG['operation_mode'] == '1') {
        if ($points_rule['hunterjobs_edit']['value'] > 0) {
            ...
            write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "修改高级职位：<strong>{$setsqlarr['jobs_name']}</strong>，({$operator}{$points_rule['hunterjobs_edit']['value']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
        }
        if ($days > 0 && $points_rule['hunterjobs_daily']['value'] > 0) {
            ...
            $user_limit_points = empty($user_limit_points['points']) ? 0 : $user_limit_points['points'];
            $operator = $points_rule['hunterjobs_daily']['type'] == "1" ? "+" : "-";
            write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "延长高级职位({$_POST['jobs_name']})有效期为{$_POST['days']}天，({$operator}{$points_day})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})");
        }
    }
    $link[0]['text'] = "职位列表";
    $link[0]['href'] = '?act=hunterjobs';
