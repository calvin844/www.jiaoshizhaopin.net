...
$smarty->assign('leftmenu', "service"); //我的账户 -> 积分操作 
if ($act == 'j_account') {
...
elseif ($act == 't_account') {
    ...
    if ($activity_id > 0) {
        ...
        $activity_name = "，套餐活动：" . $activity_info['activity_name'];
    }
    $api_path = "";
    ...
    if ($payment_name != 'remittance') {//假如是非线下支付，
    ...
    if ($setmeal && $setmeal['apply'] == "1") {
        ...
        $order_id = add_order($_SESSION['uid'], $order['oid'], $setmeal['expense'], $setmeal['days'], $payment_name, "开通服务:" . $setmeal['setmeal_name'], $timestamp, "", $setmeal['id'], "", 1);
        if ($order_id) {
    ...
    if (empty($info)) {
        ...
        if ($info['usettime'] > 0) {
            ...
            $operator = "+";
            write_memberslog($_SESSION['uid'], 1, 9001, $_SESSION['username'], "使用礼品卡({$account})充值({$operator}{$setsqlarr['giftsamount']})，(剩余积分:{$user_points}，剩余限时积分:{$user_limit_points})", 1, 1021, "礼品卡充值", "{$operator}{$setsqlarr['giftsamount']}", "{$user_points}");
            showmsg("充值成功！", 2);
        }
    }
    ...
    if ($payment_name == "points") {
        ...
        $order['oid'] = date('YmdHis', time()) . "-" . $_SESSION['uid'] . "-1"; //订单号
        $order['v_amount'] = $expense; //金额
        $order_id = add_adv_order($_SESSION['uid'], $order['oid'], $order['v_amount'], $time, $time_unit, "消费积分", $description_str, $timestamp, 1, $contact, $phone);
    } else {
        $paymenttpye = get_payment_info($payment_name);
        if (empty($paymenttpye))
            showmsg("支付方式错误！", 0);
        ...
        $order['oid'] = date('YmdHis', time()) . "-" . $_SESSION['uid'] . "-2"; //订单号
        if (strstr($paymenttpye['payment_name'], 'alipayapi-')) {
            $paymenttpye['typename'] = "alipayapi";
            $respond_name = "alipay";
        } else {
