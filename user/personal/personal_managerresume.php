...
elseif ($act=='refresh')
{
	refresh_manager_resume($_SESSION['uid'])?showmsg('操作成功！',2):showmsg('操作失败！',0);
}
//删除简历
elseif ($act=='del_resume')
{
	...
	{
	del_manager_resume($_SESSION['uid'],$_REQUEST['y_id'])?showmsg('删除成功！',2):showmsg('删除失败！',0);
	}
}
//简历列表
elseif ($act=='resume_list')
{
	...
	$smarty->assign('title','我的经理人简历 - 个人会员中心 - '.$_CFG['site_name']);
...
elseif ($act=='make1')
{
	...
	$smarty->assign('user',$user);
	$smarty->assign('title','我的高级简历 - 个人会员中心 - '.$_CFG['site_name']);
...
elseif ($act=='make1_save')
{
	...
	{	
			...
			{
			showmsg("您最多可以创建{$_CFG['manager_resume_max']} 份高级简历,已经超出了最大限制！",1);
			}
			else
			{
			...
			write_memberslog($_SESSION['uid'],2,1501,$_SESSION['username'],"创建了高级简历");
	...
	else
	{
		...
		write_memberslog($_SESSION['uid'],2,1502,$_SESSION['username'],"修改了高级简历({$_POST['title']})");
...
elseif ($act=='make2_save')
{
	...
	$resumearr['wage']=intval($_POST['wage'])?intval($_POST['wage']):showmsg('请选择期望年薪！',1);
	...
	add_manager_resume_jobs($resumepid,$_SESSION['uid'],$_POST['intention_jobs_id'])?"":showmsg('更新失败！',0);
...
elseif ($act=='make4')
{
	...
	if ($uid==0 || $pid==0) showmsg('简历不存在！',1,$link);
				...
				if (empty($resume_basic['specialty'])) showmsg("请先填写技能特长！",1,$link);
	//
...
elseif ($act=='del_education')
{
	...
	if ($db->query($sql))
	{
	check_manager_resume($_SESSION['uid'],intval($_REQUEST['pid']));//更新简历完成状态
	showmsg('删除成功！',2);
	}
	else
	{
...
elseif ($act=='make6_save')
{
	...
	if (count($resume_project)>=8) showmsg('培训经历不能超过10条！',1);
	$setsqlarr['uid']=intval($_SESSION['uid']);
	$setsqlarr['pid']=intval($_REQUEST['pid']);
 	if ($setsqlarr['uid']==0 || $setsqlarr['pid']==0 )  showmsg("参数错误！",0,$link);
	$setsqlarr['start']=trim($_POST['start'])?$_POST['start']:showmsg('请填写开始时间！',1,$link);
	$setsqlarr['endtime']=trim($_POST['endtime'])?$_POST['endtime']:showmsg('请填写结束时间！',1,$link);
	$setsqlarr['projectname']=trim($_POST['projectname'])?$_POST['projectname']:showmsg('请填写项目名称！',1,$link);
	$setsqlarr['projectposition']=trim($_POST['projectposition'])?$_POST['projectposition']:showmsg('请填写项目职务！',1,$link);
	$setsqlarr['description']=trim($_POST['description'])?$_POST['description']:showmsg('请填写项目描述！',1,$link);
...
elseif ($act=='make8_save')
{
	...
	!updatetable(table('manager_resume'),$setsqlarr," id='".intval($_REQUEST['pid'])."' AND uid='".intval($_SESSION['uid'])."'")?showmsg("保存失败！",0):'';
	check_manager_resume($_SESSION['uid'],intval($_REQUEST['pid']));
...
elseif ($act=='save_managerresume_privacy')
{
	...
	!updatetable(table('manager_resume'),$setsqlarr," uid='{$uid}' AND  id='{$pid}'")?showmsg("保存失败！",0):'';
	check_manager_resume($_SESSION['uid'],intval($_REQUEST['pid']));
	write_memberslog($_SESSION['uid'],2,1505,$_SESSION['username'],"设置高级简历隐私({$pid})");
	$link[0]['text'] = "查看简历";
	$link[0]['href'] = '?act=resume_show&pid='.$pid;
	$link[1]['text'] = "继续设置";
