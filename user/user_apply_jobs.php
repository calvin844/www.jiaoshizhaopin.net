...
if ($act == "app") {
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableall" id="app">
        ...
        <tr>
            ...
            <td>
                <textarea name="notes" id="notes"  style="border:1px solid #aaa;width:300px; height:60px; line-height:180%; font-size:12px;"></textarea>
            </td>
        </tr>
    ...
    $quick_apply = isset($_POST['quick_apply']) ? $_POST['quick_apply'] : 0;
    if ($quick_apply > 0) {
        $resume_basic = get_resume_basic($_SESSION['uid']);
        $_POST['resumeid'] = $resume_basic['id'];
        $_POST['default_resume'] = $resume_basic['default_resume'];
        if ($resume_basic['default_resume'] == 0 && $resume_basic['audit'] != 1) {
            exit("请先完善简历");
        }
    }
    $jobsid = isset($_POST['jobsid']) ? $_POST['jobsid'] : exit("投递职位错误");
    $resumeid = isset($_POST['resumeid']) ? intval($_POST['resumeid']) : exit("简历错误");
    $default_resume = isset($_POST['default_resume']) ? $_POST['default_resume'] : 0;
    ...
    foreach ($jobsarr as $jobs) {
        ...
        $addarr['resume_type'] = $default_resume;
        $addarr['notes'] = $notes;
        ...
        $apply_id = inserttable(table('personal_jobs_apply'), $addarr, 1);
        if ($apply_id > 0) {
            $sql = "select * from " . table('jiaoshi_statistics_all') . " where name = 'apply' LIMIT 1";
            $apply = $db->getone($sql);
            $setsqlarr1['total_count'] = intval($apply['total_count']) + 1;
            $setsqlarr1['new_count'] = intval($apply['new_count']) + 1;
            $wheresql1 = " name='apply' ";
            updatetable(table('jiaoshi_statistics_all'), $setsqlarr1, $wheresql1);

            $date = date('Y-m-d', time());
            $sql = "select * from " . table('jiaoshi_statistics_day') . " where name = 'apply' and date = '" . $date . "' LIMIT 1";
            $apply = $db->getone($sql);
            if (!empty($apply)) {
                $setsqlarr2['web_count'] = intval($apply['web_count']) + 1;
                $wheresql2 = " name='apply' and date='" . $date . "' ";
                updatetable(table('jiaoshi_statistics_day'), $setsqlarr2, $wheresql2);
            } else {
                $setsqlarr2['web_count'] = 1;
                $setsqlarr2['name'] = 'apply';
            ...
            if ($jobs['uid'] > 0) {
                ...
                $email = !empty($jobs['contact']['email']) ? $jobs['contact']['email'] : $comuser['email'];

                //if (1) {
                if ($mailconfig['set_applyjobs'] == "1" && $comuser['email_audit'] == "1" && $jobs['contact']['notify'] == "1") {
                    dfopen("{$_CFG['website_dir']}plus/asyn_mail.php?uid={$_SESSION['uid']}&key=" . asyn_userkey($_SESSION['uid']) . "&act=jobs_apply&jobs_id={$jobs['id']}&jobs_name={$jobs['jobs_name']}&personal_fullname={$personal_fullname}&email={$email}");
                }
                //sms	
                ...
                if ($pms_notice == '1') {
                    ...
                    $message = $personal_fullname . "申请了您发布的职位：<a href=\"{$jobs_url}\" target=\"_blank\">{$jobs['jobs_name']}</a>,<a href=\"/ProfileBank/ShowResume.shtml?ID={$resumeid}\" target=\"_blank\">点击查看</a>";
            ...
            $ch = curl_init();
            // 设置URL和相应的选项
            curl_setopt($ch, CURLOPT_URL, "http://m.jiaoshizhaopin.net/company_center/send_wechat_apply_company_core/" . $apply_id);
            ...
            // 关闭cURL资源，并且释放系统资源
            curl_close($ch);
            $website = 'http://m.jiaoshizhaopin.net/job/send_wechat_msg_apply_core/' . $apply_id;
//                //dfopen($website);
            $ch = curl_init();
            // 设置URL和相应的选项
