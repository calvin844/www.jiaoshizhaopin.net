...
if ($act == 'login' && empty($code)) {
    ...
    $smarty->assign('weiboopenid', $_SESSION["sina_access_token"]);
    ...
    if (empty($_SESSION["sina_access_token"])) {
        ...
        require_once(QISHI_ROOT_PATH . 'include/tpl.inc.php');
        $c = new SaeTClientV2($_CFG['sina_appkey'], $_CFG['sina_appsecret'], $_SESSION["sina_access_token"]);
        $ms = $c->home_timeline();
        $uid_get = $c->get_uid(); //获取u_id
        $uid = $uid_get['uid'];
        $user_message = $c->show_user_by_id($uid); //获取用户信息
        $nickname = iconv("utf-8", "gbk", $user_message['name']);
        $photo = $user_message['avatar_hd'];
    ...
    if ($userid) {
        $db->query("UPDATE " . table('members') . " SET sina_access_token = '{$_SESSION['sina_access_token']}',mobile = '{$_SESSION['mobile']}',mobile_audit = 1  WHERE uid='{$userid}' AND sina_access_token='' LIMIT 1");
